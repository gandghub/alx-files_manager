...
- `.eslintrc.js` (for linting configuration)
...
Create a class `RedisClient` to manage Redis connections. The class should include:
- A constructor that creates a Redis client.
- A function `isAlive()` to check if the Redis connection is alive.
- An asynchronous function `get(key)` to retrieve a value from Redis.
- An asynchronous function `set(key, value, duration)` to store a value with expiration.
- An asynchronous function `del(key)` to delete a key from Redis.
...
(async () => {
  ...
  await redisClient.set('myKey', 'someValue', 10);
  console.log(await redisClient.get('myKey'));
